...
class ResNet18(Model):
    def __init__(self):
        ...
        self.idblk_1 = [IdentityBlock(64, 3) for _ in range(2)]
        self.pjblk_1 = ProjectionBlock(128, 3)
        self.idblk_2 = IdentityBlock(128, 3)
        self.pjblk_2 = ProjectionBlock(256, 3)
        self.idblk_3 = IdentityBlock(256, 3)
        self.pjblk_3 = ProjectionBlock(512, 3)
        self.idblk_4 = IdentityBlock(512, 3)
    ...
    def call(self, input_tensor, training=False):
        ...
        x = self.idblk_2(x, training=training)
        # (14, 14, 128) -> (7, 7, 256)
        x = self.pjblk_2(x, training=training)
        x = self.idblk_3(x, training=training)
        # (7, 7, 256) -> (4, 4, 512)
        x = self.pjblk_3(x, training=training)
        x = self.idblk_4(x, training=training)
        # (4, 4, 512) -> (1, 1, 512)
        x = self.avgpool(x)
